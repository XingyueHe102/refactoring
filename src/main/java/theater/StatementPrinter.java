        ...
        for (Performance p : invoice.getPerformances()) {
            ...
            switch (play.getPlayType()) {
                ...
                case "comedy":
                    ...
                    break;
                default:
                    throw new RuntimeException(String.format("unknown type: %s", play.getPlayType()));
            }

            // add volume credits
            volumeCredits += Math.max(p.getAudience() - Constants.BASE_VOLUME_CREDIT_THRESHOLD, 0);
            // add extra credit for every five comedy attendees
            if ("comedy".equals(play.getPlayType())) volumeCredits += p.getAudience() / Constants.COMEDY_EXTRA_VOLUME_FACTOR;

            // print line for this order
            result.append(String.format("  %s: %s (%s seats)%n", play.getPlayName(), frmt.format(thisAmount / 100), p.getAudience()));
